...
const columnConfig = {
    open: { label: 'Open', color: 'bg-[var(--accent-yellow-soft)] text-[var(--accent-yellow)]', borderColor: 'var(--accent-yellow)' },
    in_progress: { label: 'In Progress', color: 'bg-[var(--accent-purple-soft)] text-[var(--accent-purple)]', borderColor: 'var(--accent-purple)' },
    blocked: { label: 'Blocked', color: 'bg-[var(--accent-red-soft)] text-[var(--accent-red)]', borderColor: 'var(--accent-red)' },
    done: { label: 'Done', color: 'bg-[var(--accent-green-soft)] text-[var(--accent-green)]', borderColor: 'var(--accent-green)' },
};
...
export default function KanbanColumn({ id, title, tickets, onCardClick }) {
    ...
    return (
        <div className={`flex flex-col bg-[var(--bg-raised)] rounded-[var(--radius-lg)] w-full sm:w-[320px] flex-shrink-0 h-full max-h-full overflow-hidden border transition-all duration-300 ${
            isOver
                ? 'border-[var(--accent-blue)] ring-1 ring-[var(--accent-blue)]/20 shadow-lg -translate-y-0.5'
            ...
            <div className={`flex items-center justify-between p-3.5 border-b border-t-2 transition-colors duration-300 ${isOver ? 'border-b-[var(--accent-blue)]/20' : 'border-b-[var(--border-subtle)]'} bg-[var(--bg-raised)] z-10 sticky top-0`} style={{ borderTopColor: config.borderColor }}>
                <div className="flex items-center gap-2">
                    <div className={`px-2 py-0.5 rounded-[var(--radius-sm)] text-[11px] font-bold uppercase tracking-wider ${config.color}`}>
                        {title}
                    </div>
            ...
            <div 
                ...
                className={`flex-1 p-3 overflow-y-auto space-y-3 transition-colors duration-300 custom-scrollbar ${
                    isOver ? 'bg-[var(--accent-blue-soft)]' : 'bg-[var(--bg-base)]'
                }`}
            >
                ...
                {/* Spacer at the bottom when isOver and has cards */}
                {tickets.length > 0 && isOver && (
                    <div className="h-20 border-2 border-dashed border-[var(--accent-blue)]/40 rounded-[var(--radius-md)] bg-[var(--accent-blue-soft)] transition-all duration-300" />
                )}
            </div>
        </div>
    );
